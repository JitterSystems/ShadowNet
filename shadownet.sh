																...
																for st_pid in $STAT_PIDS; do sudo kill -9 $st_pid 2>/dev/null; done
																	...
																	if [ -z "$ENGINE_VAL" ] || [ -z "$HEART_VAL" ]; then
																		...
																		if ! grep -q "ShadowNet Protocol Additions" /etc/tor/torrc; then
																			...
																			# --- THE ANTI-SPIKE CORD (STRICT 5.5MBIT TOTAL CAP) ---
																			# To fix the Rumble spike, we set the absolute ceiling of the entire interface to 5.5Mbit.
																			# This forces Rumble/Downloads to fight for space WITHIN the 5Mbit cover traffic.
																			sudo tc qdisc del dev $INT_IF root 2>/dev/null
																			sudo tc qdisc add dev $INT_IF root handle 1: htb default 10
																			# The Root Class is now hard-capped. The ISP will never see more than 5.5Mbps leaving wlo1.
																			sudo tc class add dev $INT_IF parent 1: classid 1:1 htb rate 5500kbit ceil 5500kbit
																			# ShadowNet Priority: It gets the first 5Mbit.
																			sudo tc class add dev $INT_IF parent 1:1 classid 1:5 htb rate 5000kbit ceil 5000kbit prio 0
																			# User Traffic (Rumble): It gets the leftovers and is strictly capped.
																			sudo tc class add dev $INT_IF parent 1:1 classid 1:10 htb rate 100kbit ceil 5400kbit prio 7
																			
																			iptables -t mangle -A OUTPUT -m owner --uid-owner root -j MARK --set-mark 5 
